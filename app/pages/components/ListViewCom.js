 ...
 * @flow
 * @info: ListView组件
 * 适用于滚动式页面--二维数组 每个列表带标题
 */

/*
* ListView组件是React Native中一个比较核心的组件,用途非常的广.该组件设计用来高效的展示垂直滚动的数据列表.最简单的API就是创建一个ListView.DataSource对象,
* 同时给该对象传入一个简单的数据集合.并且使用数据源(data source)实例化一个ListView组件,定义一个renderRow回调方法(该方法的参数是一个数组),该renderRow方法
* 会返回一个可渲染的组件(该就是列表的每一行的item)
* 由于平台差异性,React Native中的滚动列表组件 ListView 并没有直接映射为android 中的ListView 或 ios中的UITableView,而是在ScrollView的基础上使用JS做了
* 一次封装.这样.滚动体验部分由Native负责,而React 部分则专注于组件何时渲染,如何渲染等问题.
*
* 数据源默认的格式有三个维度:
* 第一个维度是sectionId,标识属于哪一段,可以手动指定或隐式地使用数组索引或对象的key值;
* 第二个维度是rowId,表示某个数据段下的某一个行,同样可以手动指定或隐式地使用数组索引或对象的key值
* 第三个维度是具体的数据对象,根据实际的需要而定.需要注意的是,上面只是默认的数据格式,如果它不符合实际的需求,完全可以使用自定义的数据结构.
* 唯一的区别就是需要额外指定给ListView数据源中哪些是id,哪些是rowData.
* */

/*
* DataSource 的构造函数接收以下4个参数(都是可选):
* rowHasChanged:用于数据变化的时候,计算出变化的部分,在更新时只渲染脏数据
...
* DataSource的初始化一般在constructor方法中
* 数据源确定后,下一个工作就是列表的渲染.在渲染时发挥重要作用的是renderRow属性,它接收数据源中保存的数据对象,并通过返回值确定该行该如何展现.我们可以对所有行统一进行展现,
* 也可以根据里面的字段做出不同的展现.在列表包含sectionHeader时,还需要实现renderSectionHeader方法.
*
* 注意
* 要更新dataSource中的数据,请(每次都重新)调用cloneWithRows方法(如果用到了section,则对应cloneWithRowsAndSections方法).数据源中的护具本身是不可修改的,所以请勿直接
* 尝试修改.clone方法会自动提取新数据并进行逐行对比(使用rowHasChanged方法中的策略) 这样ListView就会知道哪些重新渲染了
* */
...
var REQUEST_URL1 = require('../../json/test.json');
       ...
       return(
           <View style={styles.container}>
               <View style={styles.header}>
                   <Text style={styles.headerText}>公司人员名单</Text>
               </View>
    ...
    renderSectionHeader(sectionData,sectionID){
        return (
            <View style={styles.section}>
                <Text style={styles.text}>片头-公司名:{sectionData}-{sectionID}</Text>
            </View>
        )
    }
    //每个item信息,dom渲染
    renderRow(rowData,sectionID,rowID){
        return(
            <View style={styles.rowStyle}>
                <Text style={styles.rowText}>号:{sectionID}-{rowID}-</Text>
                <Text style={styles.rowText}>性别:{rowData.gender}</Text>
                <Text style={styles.rowText}>姓名:{rowData.name.first}{rowData.name.last}</Text>
                <Text style={styles.rowText}>邮箱:{rowData.email}</Text>
                <Text style={styles.rowText}>电话:{rowData.phone}</Text>
                <Text style={styles.rowText}>手机:{rowData.cell}</Text>
                <Text style={styles.rowText}>用户名:{rowData.username}</Text>
                <Text style={styles.rowText}>密码:{rowData.password}</Text>
    ...
    fetchData(){
        ...
        //     console.log(data)
                ...
                for(i=0;i<length;i++){
                    ...
                    rowIDs[i] = []; //rowIDs 是一个二维数组
                    for(j = 0; j < userLength; j++) {
                        user = users[j].user;
                        rowIDs[i].push(user.md5);//二维数组放的都是md5
